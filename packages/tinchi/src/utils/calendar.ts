...
export function processCalendar(rawData: Array<RawCalendar>): CalendarData {
  ...
  const calendar = rawData.map((v) => {
    ...
    // check invalid data
    if (defaultName == '') throw new Error('invalid subject name: data has empty subject name')
  ...
  if (!minTime || !maxTime) throw new Error(`invalid data - empty min/max time`)

  for (let i = minTime; i <= maxTime; i += 86400000 /* 1 day */) dateList.push(i)
  ...
  for (const item of data) {
    if (!(item.nameOnly in result)) {
      result[item.nameOnly] = <CalendarGroupBySubjectNameDetail>{
        majors: item.majors,
        classes: <CalendarGroupByClass>{},
        id: String(generatedId).padStart(3, '0')
      }
      generatedId++
    }
