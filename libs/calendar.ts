...
export async function processCalendar(rawHtml: string) {
  ...
  return await new Promise((resolve, reject) => {
    const worker = createInlineWorker(
      (_rawHtml: { data: any }) =>
        self.postMessage(restructureTKB(_rawHtml.data)),
      restructureTKB
    );
    worker.onmessage = (res) =>
      resolve(
        res.data ? res.data : res.data === false ? { data_subject: [] } : null
      );
...
export function processMainForm(rawHtml: any) {
  ...
  return mainForm
    ? mainForm
        .serializeArray()
        .reduce((o: any, kv: any) => ({ ...o, [kv.name]: kv.value }), {})
    : {};
}
...
export function restructureTKB(data: any) {
  ...
  const data_subject: any = Array.prototype.map.call(
    content_data,
    function (td) {
      ...
      if (temp_dia_diem) {
        ...
        temp_dia_diem_season_index.forEach(
          (child_item: any) =>
            (temp_dia_diem = temp_dia_diem.replace(
              child_item,
              "\n" + child_item
            ))
        );
      ...
      const temp_thoi_gian = td[
        header_data.indexOf(categories.thoi_gian)
      ...
      temp_thoi_gian.forEach((item: any, index: any) => {
        ...
        // remove trash
        item[1] = `${item[1].substr(3, 2)}/${item[1].substr(
          0,
          2
        )}/${item[1].substr(6, 4)}`;
        item[2] = `${item[2].substr(3, 2)}/${item[2].substr(
          0,
          2
        )}/${item[2].substr(6, 4)}`;
  ...
  for (const week of days_outline) {
    for (const day of week) {
      day.shift = [...(Array(16).keys() as any)].map((shift) => {
...
export function exportToGoogleCalendar(student: string | null, calendar: any) {
  ...
  calendar.data_subject.forEach((week: any) => {
    for (const day of week) {
      ...
      day.shift.forEach((shift: any, shift_index: number) => {
        if (shift.content) {
          result += `BEGIN:VEVENT\nDTSTART:${moment(timeIter).format(
            "YYYYMMDD"
          )}T${time_sift_table[shift_index + 1].start}Z\n`;
  ...
  link.setAttribute(
    "download",
    `${student ? student.split(" - ")[0] : "tkb_export"}.ics`
  );
